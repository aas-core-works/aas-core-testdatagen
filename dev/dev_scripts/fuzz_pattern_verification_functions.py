...
def main() -> None:
    ...
    parser.add_argument(
        "--model_path",
        help="path to the meta-model",
        default=str(repo_root / "dev" / "test_data" / "meta_model" / "v3_1.py"),
    )
    ...
    for verification_function in symbol_table.verification_functions:
        ...
        for i in range(10):
            ...
            print(f"    (Filenameable('fuzzed_{(i + 1):02d}'), {text!r}),")
        print("  ]")
        print()
        ...
        for _ in range(1000):
            ...
            if pattern_re.match(text) is None:
                print(
                    f"    (Filenameable('negatively_fuzzed_{(count + 1):02d}'), {text!r}),"
                )
                count += 1
                observed.add(text)
